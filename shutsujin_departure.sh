...
get_role_cmd() {
    ...
    default_role_cmd "$provider" "$role"
}
...
SHOGUN_PROVIDER_LABEL=$(get_provider_label "$SHOGUN_PROVIDER")
KARO_PROVIDER_LABEL=$(get_provider_label "$KARO_PROVIDER")
ASHIGARU_PROVIDER_LABEL=$(get_provider_label "$ASHIGARU_PROVIDER")

SHOGUN_BINARY=$(get_provider_binary "$SHOGUN_PROVIDER")
KARO_BINARY=$(get_provider_binary "$KARO_PROVIDER")
ASHIGARU_BINARY=$(get_provider_binary "$ASHIGARU_PROVIDER")
...
# ═══════════════════════════════════════════════════════════════════════════════
# STEP 4: multiagentセッション作成（karo + ashigaru）
# ═══════════════════════════════════════════════════════════════════════════════
# tmux の存在確認
...
TOTAL_PANES=$((ASHIGARU_COUNT + 1))
log_war "⚔️ 家老・足軽の陣を構築中（家老1 + 足軽${ASHIGARU_COUNT}）..."
...
if [ "$SETUP_ONLY" = false ]; then
    ...
    for provider in "${unique_providers[@]}"; do
        binary=$(get_provider_binary "$provider")
        label=$(get_provider_label "$provider")
        if ! command -v "$binary" &> /dev/null; then
            log_info "⚠️  ${binary} コマンドが見つかりません (${label} CLI)"
            missing_cli=true
        fi
    done
    ...
    # 家老
    tmux send-keys -t "multiagent:0.0" "$KARO_CLI_CMD"
    ...
    if [ "$SHOGUN_PROVIDER" = "claude" ]; then
        echo "  ${SHOGUN_PROVIDER_LABEL} の起動を待機中（最大30秒）..."

        # 将軍の起動を確認（最大30秒待機）
        for i in {1..30}; do
            if tmux capture-pane -t shogun -p | grep -q "bypass permissions"; then
                echo "  └─ 将軍の ${SHOGUN_PROVIDER_LABEL} 起動確認完了（${i}秒）"
                break
            fi
            sleep 1
        done
    else
        echo "  ${SHOGUN_PROVIDER_LABEL} CLI の起動を確認中..."
        sleep 2
    fi
...
echo "     └─────────────────────────────┘"
echo ""
echo "     【multiagentセッション】家老・足軽の陣（家老1 + 足軽${ASHIGARU_COUNT}ペイン）"
echo "     ┌──────────────────────────────────────────┐"
echo "     │  Pane 0: karo (家老)                      │"
echo "     │  Pane 1-${ASHIGARU_COUNT}: ashigaru1..${ASHIGARU_COUNT} (足軽) │"
echo "     └──────────────────────────────────────────┘"
echo ""
...
if [ "$SETUP_ONLY" = true ]; then
    echo "  ⚠️  セットアップのみモード: AI CLI は未起動です"
    echo ""
    echo "  手動でAI CLI を起動するには:"
    echo "  ┌──────────────────────────────────────────────────────────┐"
    echo "  │  # 将軍を召喚                                            │"
